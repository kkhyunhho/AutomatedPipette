...
| `setup_docker_ble.sh` | Bring up the BlueZ stack inside the container (D-Bus system bus + bluetoothd + power on hci0), then print the bonding steps. | Requires the container to run with `--network host`; with its own netns, `AF_BLUETOOTH` sockets fail with EAFNOSUPPORT even though `/sys/class/bluetooth/hci0` is visible and the container is privileged. `--privileged` alone is insufficient. |
