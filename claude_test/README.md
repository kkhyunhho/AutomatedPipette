...
| `forward_pipetting_usb.py` | Interactive forward pipetting with real liquid over USB via the `picus2` module (`Picus2Client.over_serial`). Pauses (Enter) for mount-tip / dip / move. Run in a terminal with `PYTHONPATH=src`. | Pending hardware run (500 uL, speed 7): authorize (no tip) -> mount+dip -> aspirate -> move -> blow-out. |
